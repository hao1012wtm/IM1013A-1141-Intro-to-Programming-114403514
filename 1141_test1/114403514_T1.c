...
int main (void){
    ...
    while(selection!=-1){
        if(selection==1){
            ...
            // to decide the quantity discount factor
            if(ticket<=5){
                factor1=1.00;
            }
            else if(ticket<=10){
            ...
            total=adult*200+student*150; //compute the total price
            printf("\n--- Quotation Summary ---\n");
            ...
            total=total*factor1*factor2; //compute the final total price(including discount)
            ...
            // to decide the period,fee,and rate
            ...
            repayment=principal*pow(1.00+rate,period); //compute the repayment
            totalfee=fee*period;
